...
$C_{11}$ is the smallest constant such that, for every $n\ge 1$ and every function $f$:{-1,1}^n $\to \mathbb{R}$

$$
\mathbb{E}\bigl|f(x)-\mathbb{E}f(x)\bigr|\ \le\ C_{11}\mathbb{E}|\nabla f|(x),
$$

where $x=(x_1,\dots,x_n)$ is uniform on {-1,1}^n and

$$
|\nabla f|(x)=\Bigl(\sum_{j=1}^n |D_j f(x)|^2\Bigr)^{1/2},\qquad
D_j f(x)=\frac{f(x)-f(x^{(j)})}{2},
$$

with
$x^{(j)}=(x_1,...,x_{j-1},-x_j,x_{j+1},...,x_n).$
...
| $\pi/2-\delta$ for some $\delta>0$ | [ILvHV2019] | First proof that $C_8$ is strictly smaller than $\pi/2$. |
...
- It remains open whether $C_{11}$ equals $\sqrt{\pi/2}$.
